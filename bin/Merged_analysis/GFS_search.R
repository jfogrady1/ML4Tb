...
# WIARDA
wiarda = fread(args[4], sep = "\t") %>% select(-1)
wiarda_labels = fread(args[5], sep = "\t")
...
# Kirsten data
Kirsten <- fread(args[6], sep = "\t") %>% select(-1)
Kirsten <- as.matrix(Kirsten)
rownames(Kirsten) <- ensemble$gene_name
kirsten_labels <- fread(args[7]) %>% as.data.frame() #%>% select(1,2)
...
# Kirsten PBL data
Kirsten_pbl <- fread(args[8], sep = "\t") %>% select(-1)
Kirsten_pbl <- as.matrix(Kirsten_pbl)
rownames(Kirsten_pbl) <- ensemble$gene_name
kirsten_pbl_labels <- fread(args[9]) %>% as.data.frame()
...
CPM_counts <- read.table(args[1],check.names=F)
DE_genes <- fread(args[10]) %>% filter(padj < 0.05) %>% filter(baseMean > 100) %>% select(Symbol)
...
train_set <- read.table(file = args[12])
...
metadata_train <- metadata_all %>% filter(rownames(.) %in% colnames(CPM_counts))
metadata_train$Study <- gsub("1_", "", metadata_train$Study)
metadata_train$Study <- gsub("2_", "", metadata_train$Study)
CPM_counts <- CPM_counts %>% select(-c("Condition"))
GFS_train_counts = as.data.frame(t(CPM_counts), stringsAsFactors = FALSE)
GFS_train_counts[] = lapply(GFS_train_counts, type.convert, as.is = TRUE)

as.matrix(GFS_train_counts)
DE_genes <- DE_genes[DE_genes %in% rownames(GFS_train_counts)]

ROC = greedy_forward_search(gene.list = DE_genes, cpm_matrix = as.matrix(GFS_train_counts), metadata = train_set, 
                            de_results = DE_results[DE_results$Symbol %in% rownames(GFS_train_counts),])


write.table(ROC, file = args[13], sep = "\t", row.names = F, quote = FALSE)
GFS_1_genes <- ROC$combination[(dim(ROC)[1] -1)]
GFS_1_genes <- str_split_1(GFS_1_genes, pattern = "_")


DE_results1 = DE_results %>% filter(!(Symbol %in% GFS_1_genes))
DE_results1 = DE_results1 %>% filter(Symbol %in% rownames(GFS_train_counts))


ROC2 = greedy_forward_search(gene.list = DE_results1$Symbol, cpm_matrix = as.matrix(GFS_train_counts)[!rownames(as.matrix(GFS_train_counts)) %in% GFS_1_genes, ], metadata = train_set, de_results = DE_results1)
write.table(ROC2, file = args[14], sep = "\t", row.names = F, quote = FALSE)
GFS_2_genes <- ROC2$combination[(dim(ROC2)[1] -1)]
...
Scores_for_plotting_combined %>%
  ...
  add_significance() %>% arrange(desc(Study)) %>% write.table(., args[16], sep = "\t")  
...
GFS_train_combined %>% write.table(., args[17], sep = "\t")
...
ggplot(GFS_train_combined, aes(x = Folds, y = AUC, col = set, group = factor(set))) + geom_pointrange(aes(ymin = AUC_CI_lower, ymax = AUC_CI_upper, color = set),
                                                                                                      position = position_dodge(width = 0.2)) + ylim(0.85,1) + 
...
table(Scores_for_plotting_combined$Study)
Scores_for_plotting_combined$Study <- as.character(Scores_for_plotting_combined$Study)
Scores_for_plotting_long <- rbind(Scores_for_plotting_combined, Scores_for_plotting_combined)
Scores_for_plotting_long$Study <- c(Scores_for_plotting_combined$Study, rep("Overall", dim(Scores_for_plotting_combined)[1]))
#cores_for_plotting_combined2 <- factor(Scores_for_plotting_combined2$Study, levels = c("OGrady", "Wiarda", "Mcloughlin", "Mcloughlin_pbl", "Overall"))
...
overal_thresh_vec = GFS_thresholds_overall %>% filter(Sens > 0.9) %>% filter(Combined == max(Combined)) %>% filter(Sens == max(Sens))
GFS_thresholds_overall
overal_thresh_vec$Threshold
...
ci_vals <- ci.se(roc(Condition ~ Score, data = Scores_for_plotting_long, legacy.axes = TRUE), specificities = seq(0, 1, 0.01))
df <- data.frame(
...
GFS_train_score + 
  ...
  annotate("point", x = 1 - overal_thresh_vec$Spec, y = overal_thresh_vec$Sens, colour = "darkred", size = 5, shape = 18)
...
IGRA = fread(args[20])
Scores_for_plotting <- left_join(Scores_for_plotting_combined, IGRA, by = c("Sample" = "Sample ID"))
IGRA_correlation = Scores_for_plotting %>% group_by(Set) %>% summarise(
                                                                       P = cor.test(`IFGA_bovine - IFGA_avian`, Score, method = "spearman", exact = FALSE)$p.value,
                                                                       rho = cor.test(`IFGA_bovine - IFGA_avian`, Score, method = "spearman", exact = FALSE)$estimate) %>%
  mutate(
    # Add coordinates for text placement
    x_pos = 2.0,
    y_pos_rho = 2500,
    y_pos_p = 2200,
...
ggplot(Scores_for_plotting, aes(x = Score, y = `IFGA_bovine - IFGA_avian`)) + geom_point(size = 3, aes(col = Condition)) +
  ...
  geom_vline(xintercept =  overal_thresh_vec$Threshold, col = "darkgrey", linetype = "dashed", linewidth = 1) +
  theme(axis.text.x = element_text(angle = 0, size = 15, colour = "black"),
        axis.text.y = element_text(angle = 0, size = 15, colour = "black"),
        axis.title.x = element_text(size = 18, colour = "black"),
        axis.title.y = element_text(size = 18, color = "black"),
        ...
        legend.margin = margin(6, 6, 6, 6),
        legend.box.background = element_rect(color="black", size=2)) +
  # Use geom_text with the new data frame for the rho labels
  geom_text(
    data = IGRA_correlation,
    aes(x = x_pos, y = y_pos_rho, label = rho_label),
    parse = FALSE,
    size = 5,
    hjust = 0 # Adjust horizontal justification
  ) +
  # Use geom_text with the new data frame for the p-value labels
  geom_text(
    data = IGRA_correlation,
    aes(x = x_pos, y = y_pos_p, label = p_label),
    parse = FALSE,
    size = 5,
    hjust = 0 # Adjust horizontal justification
  ) + facet_wrap(~factor(Set))
...
Test_data = as.data.frame(t(read.table(args[22], sep = "\t", check.names=F)))
...
ggplot(Test_Scores_for_plotting_combined, aes(y = Scaled_Score, x = factor(Study, levels = c("OGrady","Mcloughlin_pbl", "Wiarda", "Mcloughlin")), fill = Set, shape = Condition, col = Condition)) + 
  geom_boxplot(position = position_dodge2(width = 2, padding = 1.9), outlier.colour = NA, alpha = 1) + 
  geom_point(position = position_jitterdodge(jitter.width = 0.15), alpha = 0.5, size = 3) + 
  scale_colour_manual(values = c("#2166ac", "#b2182b")) +
  scale_fill_brewer(palette = "Set2") +
...
Test_Scores_for_plotting_combined %>% filter(Study != "Wiarda") %>%
  ...
  wilcox_test(Score ~ Condition) %>%
  adjust_pvalue(method = "BH") %>%  # Optional: adjust p-values
  add_significance() %>% arrange(desc(Study)) %>% write.table(., args[24], sep = "\t")     
...
Test_GFS_thresholds_Set1 = roc(Condition ~ Scaled_Score, data = Test_Scores_for_plotting_combined[Test_Scores_for_plotting_combined$Set == "Set1",]) 
Test_GFS_thresholds_Set2 = roc(Condition ~ Scaled_Score, data = Test_Scores_for_plotting_combined[Test_Scores_for_plotting_combined$Set == "Set2",])
Test_GFS_thresholds_Set3 = roc(Condition ~ Scaled_Score, data = Test_Scores_for_plotting_combined[Test_Scores_for_plotting_combined$Set == "Set3",]) 
...
Test_GFS_thresholds_Set3

set1_thresh_vec = Test_GFS_thresholds_Set1 %>% filter(Sensitivity > 0.9) %>% filter(Combined == max(Combined)) # 1 -0.3459629   0.9117647   0.8529412 Set1 1.764706
set2_thresh_vec = Test_GFS_thresholds_Set2 %>% filter(Sensitivity > 0.9) %>% filter(Combined == max(Combined)) # -0.2379047   0.9411765   0.7647059 Set2 1.705882
set3_thresh_vec = Test_GFS_thresholds_Set3 %>% filter(Sensitivity > 0.9) %>% filter(Combined == max(Combined))#  -0.3740885   0.9411765   0.7647059 Set3 1.705882

threshold_set1 = set1_thresh_vec$Threshold
threshold_set2 = set2_thresh_vec$Threshold
threshold_set3 = set3_thresh_vec$Threshold
...
test_set = read.table(args[25], header = T, row.names = 1)
head(test_set)
table(test_set$Study)
...
library(pheatmap)
pdf(args[26], width = 15, height = 20)
...
ggsave(args[28], width = 15, height = 12, dpi = 600)
